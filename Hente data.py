...
player_stats = groundduel.groupby(['player.name','team.name','label']).agg(
    ...
    totalDuels=('groundDuel.stoppedProgress', 'count')
).reset_index()

# Calculate the percentage of True values for each player
player_stats['stoppedProgressPercentage'] = (player_stats['stoppedProgressCount'] / player_stats['totalDuels']) * 100
player_stats['recoveredPossessionPercentage'] = (player_stats['recoveredPossessionCount'] / player_stats['totalDuels']) * 100
...
connection_string = 'SharedAccessSignature=sv=2020-08-04&ss=f&srt=sco&sp=rl&se=2025-01-11T22:47:25Z&st=2022-01-11T14:47:25Z&spr=https&sig=CXdXPlHz%2FhW0IRugFTfCrB7osNQVZJ%2BHjNR1EM2s6RU%3D;FileEndpoint=https://divforeningendataout1.file.core.windows.net/;'
share_name = 'divisionsforeningen-outgoingdata'
dir_path = 'KampData/Sæson 24-25/Superliga/'
...
# Save to CSV
player_stats.to_csv('groundduels_per_player superliga.csv', index=False)

events = events[['id','player.name','matchId', 'player.id', 'team.name', 'matchId','pass.recipient.name','pass.accurate','label','date','minute','second','groundDuel','aerialDuel','infraction','carry','type.primary','type.secondary','location.x','location.y','pass.endLocation.x','pass.endLocation.y','carry.endLocation.x','carry.endLocation.y','shot.xg','possession.types','possession.eventsNumber','possession.eventIndex','possession.startLocation.x','possession.startLocation.y','possession.endLocation.x','possession.endLocation.y','possession.team.name','possession.attack.xg']]
transitions = events[events['possession.eventsNumber']<=15]
exclude_types = ['throw_in', 'set_piece_attack', 'free_kick', 'corner']
events = events.to_csv('events superliga.csv', index=False)
def convert_to_list(value):
    if isinstance(value, str):
        return ast.literal_eval(value)
    return value
...
transitions = transitions[~transitions['possession.types'].apply(exclude_possession_types)]
transitions.to_csv('transitions superliga.csv', index=False)
...
# Convert the events_list to a DataFrame
matchstats = pd.DataFrame(players_list)
matchstats.to_csv('matchstats superliga.csv', index=False)
import pandas as pd
import ast
matchstats = pd.read_csv(r'C:\Users\SéamusPeareBartholdy\Documents\GitHub\AC-Horsens-U19\matchstats superliga.csv')
matchstats.rename(columns={'playerId': 'player.id'}, inplace=True)
events = pd.read_csv(r'C:\Users\SéamusPeareBartholdy\Documents\GitHub\AC-Horsens-U19\events superliga.csv')
name_and_id = events[['player.name','player.id','team.name','date','matchId','label']]
name_and_id = name_and_id.drop_duplicates()
matchstats = matchstats.merge(name_and_id)
data = matchstats['positions']
...
xg = events[events['shot.xg'] > 0]
xg = xg[['player.name','team.name','label','date','location.x','location.y','minute','shot.xg']]
xg.to_csv('xg superliga.csv',index=False)

xg_agg = xg[xg['label'].str.contains('Horsens')]
xg_agg.to_csv('xg_agg superliga.csv', index=False)
...
terr_poss = terr_poss.groupby('label').apply(apply_for_each_label).reset_index(drop=True)
terr_poss = terr_poss.to_csv('terr_poss superliga.csv', index=False)
...
df_ppdabeyond40['PPDA'] = df_ppdabeyond40['opponents passes'] / df_ppdabeyond40['defensive actions']
df_ppda = df_ppdabeyond40[['label', 'team.name','date', 'PPDA']]
df_ppda.to_csv('PPDA superliga.csv', index=False)
...
penalty_area_entries = events[['player.name','team.name', 'label','pass.recipient.name','date','location.x', 'location.y','pass.endLocation.x', 'pass.endLocation.y', 'carry.endLocation.x', 'carry.endLocation.y', 'penalty_area_entry']]
penalty_area_entries = penalty_area_entries[penalty_area_entries['penalty_area_entry'] == True]
penalty_area_entries.to_csv('penalty_area_entries superliga.csv', index=False)
penalty_area_entry_counts = penalty_area_entries.groupby(['label','date', 'team.name'])['penalty_area_entry'].sum()

penalty_area_entry_counts = penalty_area_entry_counts.reset_index()

penalty_area_entry_counts = penalty_area_entry_counts.rename(columns={'penalty_area_entry': 'count'})
penalty_area_entry_counts.to_csv('penalty_area_entry_counts superliga.csv', index=False)
...
dangerzone_entries = events[['player.name','team.name', 'label','pass.recipient.name','date','location.x', 'location.y','pass.endLocation.x', 'pass.endLocation.y', 'carry.endLocation.x', 'carry.endLocation.y', 'dangerzone_entry']]
dangerzone_entries = dangerzone_entries[dangerzone_entries['dangerzone_entry'] == True]
dangerzone_entries.to_csv('dangerzone_entries superliga.csv', index=False)
