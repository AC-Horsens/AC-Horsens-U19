...
def Process_data_spillere(events,df_xg,df_matchstats,groundduels):
    ...
    df_scouting = groundduels.merge(df_scouting,on=['SHORTNAME','TEAMNAME', 'MATCHLABEL'],how='right')
...
def player_data():
    ...
    with col1:
        ...
        team_name = st.selectbox('Choose team', sorted(set([name for name in events['TEAMNAME'].dropna() if isinstance(name, str)])))

    events = events[events['TEAMNAME'] == team_name]
    with col2:
        # Filter out non-string values, drop NaN, ensure uniqueness, and sort alphabetically
        player_name = st.selectbox('Choose player', sorted(set([name for name in events['SHORTNAME'].dropna() if isinstance(name, str)])))

    st.title(f'{player_name} dashboard')    
    df = events[(events['SHORTNAME'] == player_name)|(events['pass.recipient.name'] == player_name)]
    df = df[~df['type.primary'].isin(['corner', 'free_kick', 'throw_in'])]
    df['DATE'] = pd.to_DATEtime(df['DATE'])
    df = df.sort_values(by='DATE',ascending=False)
    kampe = df['MATCHLABEL'].unique()
    kampvalg = st.multiselect('Choose matches', kampe, kampe[0:3])
    df = df[df['MATCHLABEL'].isin(kampvalg)]
    df_matchstats_player = df_matchstats[(df_matchstats['SHORTNAME'] == player_name) & (df_matchstats['MATCHLABEL'].isin(kampvalg))]
    ...
    balanced_central_defender_df = balanced_central_defender_df[(balanced_central_defender_df['MATCHLABEL'].isin(kampvalg)) & (balanced_central_defender_df['SHORTNAME'] == player_name)]
    fullbacks_df = fullbacks_df[(fullbacks_df['MATCHLABEL'].isin(kampvalg)) & (fullbacks_df['SHORTNAME'] == player_name)]
    number6_df = number6_df[(number6_df['MATCHLABEL'].isin(kampvalg)) & (number6_df['SHORTNAME'] == player_name)]
    number8_df = number8_df[(number8_df['MATCHLABEL'].isin(kampvalg)) & (number8_df['SHORTNAME'] == player_name)]
    number10_df = number10_df[(number10_df['MATCHLABEL'].isin(kampvalg)) & (number10_df['SHORTNAME'] == player_name)]
    winger_df = winger_df[(winger_df['MATCHLABEL'].isin(kampvalg)) & (winger_df['SHORTNAME'] == player_name)]
    classic_striker_df = classic_striker_df[(classic_striker_df['MATCHLABEL'].isin(kampvalg)) & (classic_striker_df['SHORTNAME'] == player_name)]
    balanced_central_defender_df = balanced_central_defender_df.drop(columns=['SHORTNAME', 'TEAMNAME', 'POSITION1CODE'],errors = 'ignore')
    fullbacks_df = fullbacks_df.drop(columns=['SHORTNAME', 'TEAMNAME', 'POSITION1CODE'],errors = 'ignore')
    number6_df = number6_df.drop(columns=['SHORTNAME','TEAMNAME','POSITION1CODE'],errors = 'ignore')
    number8_df = number8_df.drop(columns=['SHORTNAME','TEAMNAME','POSITION1CODE'],errors = 'ignore')
    number10_df = number10_df.drop(columns=['SHORTNAME', 'TEAMNAME', 'POSITION1CODE'],errors = 'ignore')
    winger_df = winger_df.drop(columns=['SHORTNAME', 'TEAMNAME', 'POSITION1CODE'],errors = 'ignore')
    classic_striker_df = classic_striker_df.drop(columns=['SHORTNAME', 'TEAMNAME', 'POSITION1CODE'],errors = 'ignore')
