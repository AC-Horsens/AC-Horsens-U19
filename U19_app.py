...
def sportspsykologiske_målinger():
    ...
    if password == "ACHORSENSU19":
        ...
        # Function to extract numeric value from mixed responses
        def extract_numeric(value):
            if isinstance(value, str):
                match = re.match(r'^(\d+)', value.strip())
                if match:
                    return int(match.group(1))  # Extract and return the first numeric character
            return pd.NA  # Return NA if not numeric
        ...
        # Extract numeric responses for all answer columns
        ...
        columns_to_reverse = [col for col in merged_df.columns if 'Frustration' in col or 'Indifference' in col]
